...
class AwesomeStatusBarApp(rumps.App):
    ...
    def start_new_sess(self):
        self.status = "inter"
        test = rumps.Window(message="What's your goal?", title='Start Pomodoro Session', default_text='', ok="Start", cancel="Procrastinate", dimensions=(320,80))
        test.icon = "./invisible.png"
        response = test.run()
    ...
    def end_sess(self):
        ...
        self.status = "inter"
        self.title = "--:--"
        test = rumps.Window(message="Goal: "+self.init_goal+"\nDid you achieve your goal?", title='Session Done!', default_text='', ok="Start Break", cancel="Finish", dimensions=(320,80))
        test.icon = "./invisible.png"
        response = test.run()
        # log the session
    ...
    @rumps.timer(1)  # create a new thread that calls the decorated function every second
    ...
    def g_test(self, start_t, end_t, title, outcome):
        ...
        event = {
          ...
          'start': {
            'dateTime':  start_t.strftime("%Y-%m-%dT%H:%M:%S"),
            'timeZone': 'Europe/London',
          },
          'end': {
            'dateTime':  end_t.strftime("%Y-%m-%dT%H:%M:%S"),
            'timeZone': 'Europe/London',
          },
        }
